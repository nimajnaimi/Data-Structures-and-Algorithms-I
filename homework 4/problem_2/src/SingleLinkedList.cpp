...
template <class T>
ListNode<T> *SingleLinkedList<T>::_reverse_recursive(ListNode<T> *curr) {
    if (curr == nullptr || curr->next == nullptr) {
        head->next = curr; // set the head to the last node in the list
        return curr;
    }
    ListNode<T> *temp = _reverse_recursive(curr->next); // stores pointer to the next node
    temp->next = curr; // points the next node to the current node
    curr->next = nullptr; // points the last node to null
    return curr;
}
